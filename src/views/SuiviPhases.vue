...
const planAql = computed(() => {
  ...
  return planPourLot(taille, aqlForm.aql, aqlForm.niveau)
})
const verdictAql = computed(() => {
  const p = planAql.value
  return p ? verdictClasse(aqlForm.defauts, p.ac) : null
})
...
async function enregistrerAql() {
  ...
  const r = await supabase.from('controles_aql').insert({
    ...
    aql: aqlForm.aql, ac: p.ac, re: p.re, defauts: Number(aqlForm.defauts) || 0,
  ...
  aqlForm.defauts = 0; aqlForm.commentaire = ''
  await chargerAql()
}
      ...
      <template v-if="lotId">
        ...
        <section class="card aql-card" v-if="phasesAql.length">
          ...
          <div v-if="planAql && peutEditer" class="aql-pied">
            ...
            <span class="aql-verdict" :class="verdictAql === 'Refusé' ? 'av-ko' : 'av-ok'">{{ verdictAql }}</span>
            <button class="btn" @click="enregistrerAql">Enregistrer le contrôle</button>
          </div>
          <table v-if="controlesAql.length" class="aql-hist">
            <thead><tr><th>Date</th><th>Étape</th><th class="anum">Lot</th><th class="anum">Éch.</th><th class="anum">AQL</th><th class="anum">Ac / Re</th><th class="anum">Défauts</th><th>Verdict</th><th>Par</th><th></th></tr></thead>
            <tbody>
              <tr v-for="x in controlesAql" :key="x.id">
                ...
                <td class="anum">{{ x.aql }}</td>
                <td class="anum">{{ x.ac }} / {{ x.re }}</td>
                <td class="anum">{{ x.defauts }}</td>
...
.aql-hist .anum { text-align: right; }
...
.aql-def { display: flex; align-items: center; gap: 7px; font-size: 12px; font-weight: 700; color: #334155; white-space: nowrap; }
