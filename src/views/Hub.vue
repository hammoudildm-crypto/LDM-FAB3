...
/* Ligne / convoyeur */
.ligne-wrap { min-height: 360px; display: flex; flex-direction: column; justify-content: center; position: relative; padding: 24px 62px 78px; }
.dir { position: absolute; top: 8px; right: 62px; font-size: 11px; font-weight: 700; color: #94a3b8; letter-spacing: 2px; text-transform: uppercase; }
...
/* Toutes les pages */
.ma-head { max-width: 1180px; margin: 8px auto 12px; padding: 0 20px; font-size: 13px; font-weight: 800; letter-spacing: 1.5px; text-transform: uppercase; color: #94a3b8; }
.menu-all { max-width: 1180px; margin: 0 auto; padding: 0 20px; display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; align-items: start; }
.ma-group { background: rgba(255,255,255,.78); border: 1px solid #e2e8f0; border-radius: 16px; padding: 18px 20px; box-shadow: 0 6px 18px rgba(30,41,59,.06); }
...
.ma-link:hover { background: #eef3f9; color: #0f172a; padding-left: 15px; }

@keyframes blink { 0%,100% { opacity: 1; } 50% { opacity: .4; } }

@media (max-width: 820px) {
  .ph { padding: 18px 20px; flex-wrap: wrap; gap: 10px; } .ph-sub { display: none; }
  .ligne-wrap { min-height: 0; padding: 20px 22px 40px; }
  ...
  .menu-all { grid-template-columns: 1fr; } .ma-head, .menu-all { max-width: none; }
